...
public record TrendPointDto(string Period, double Assets, double FinancialAssets, double Liabilities, double NetWorth);
...
public record AssetPerformanceDto(
    ...
    string Category,
    string? Owner,
    List<YearValueDto> Years);

public record YearValueDto(string Year, double Value);
